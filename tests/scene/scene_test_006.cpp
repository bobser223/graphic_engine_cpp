...
int main() {
    ...
    { // Context for RAII shaders, textures and model
        ...
        Shader shader(PROJECT_PATH / "tests" / "scene" / "scene_test_006.vert",
                      PROJECT_PATH / "tests" / "scene" / "scene_test_006.frag");

        auto texture = std::make_shared<Texture>((PROJECT_PATH / "data/texture1.png").string(), TextureType::Diffuse);


        Model loaded_model = ModelLoader::loadModel(PROJECT_PATH / "data/bmw/bmw.obj",
        ...
        Node& model_node_2 = root.createChild(&loaded_model);
        model_node_2.transform_.position_ = glm::vec3(0.0f, -0.42f, +4.0f);
        model_node_2.transform_.scale_ = glm::vec3(0.030f);
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            shader.setUniform("light.position", glm::vec3(2.0f, 4.0f, 2.0f));
            shader.setUniform("light.color", glm::vec3(1.0f, 1.0f, 1.0f));
            shader.setUniform("light.intensity", 1.0f);
